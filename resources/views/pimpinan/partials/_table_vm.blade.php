<div class="card card-primary mb-4">
    <div class="card-header">
        <h3 class="card-title">Virtual Meeting</h3>
    </div>
    <div class="card-body">
                        ...
                        <td class="text-center align-middle">
                            @if ($item->surat_permohonan)
                                <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#modalSuratVm{{ $item->id }}">
        ...
        @foreach ($data as $item)
            ...
            @if ($item->surat_permohonan)
                ...
                <div class="modal fade" id="modalSuratVm{{ $item->id }}" tabindex="-1"
                    aria-labelledby="modalSuratLabel{{ $item->id }}" aria-hidden="true">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item"><strong>Topik:</strong> {{ $item->topik_meeting }}</li>
                                <li class="list-group-item"><strong>Waktu:</strong> {{ $item->waktu_pelaksanaan }}
                                </li>
                                <li class="list-group-item"><strong>Jumlah Partisipan:</strong>
                                    {{ $item->jumlah_partisipan }}</li>
                                <li class="list-group-item"><strong>Durasi:</strong> {{ $item->durasi_meeting }}</li>
                                <li class="list-group-item"><strong>Lokasi:</strong> {{ $item->lokasi_meeting }}</li>
                            </ul>
                        </div>
                        <div class="modal-footer justify-content-between">
                            <button type="button" class="btn btn-secondary btn-sm"
