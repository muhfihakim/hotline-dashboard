<x-layouts.modern>
  <div class="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h2 class="text-xl font-heading font-bold text-slate-800">Virtual Meeting</h2>
      <p class="text-sm text-slate-500">Daftar permohonan fasilitas virtual meeting.</p>
    </div>
    <div class="flex items-center gap-2 text-sm text-slate-500">
      <span>Home</span> <i data-lucide="chevron-right" class="w-3 h-3"></i> <span class="font-semibold text-slate-700">Virtual Meeting</span>
    </div>
  </div>
  ...
  <div id="table-container" class="bg-white rounded-box shadow-soft border border-slate-200 overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
      <h3 class="font-heading font-bold text-sm text-slate-800">Data Virtual Meeting</h3>
    </div>
      ...
      <table class="w-full text-left border-collapse data-table">
        <thead>
          <tr>
            ...
            <th class="px-5 py-3 border-b border-slate-100 bg-slate-50/50 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Fasilitas</th>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-xs text-gray-500">{{ $item->link_operator }}</td>
            <td>
              @if($item->status == '1')
            ...
            <td class="flex gap-2 items-center">
              <button onclick="openModalPelaksanaan('{{ $item->id }}')" class="p-1.5 rounded-lg hover:bg-brand-50 text-brand-500" title="Lihat Detail"><i data-lucide="info" class="w-4 h-4"></i></button>
          ...
          @empty
          <tr><td colspan="6" class="text-center py-4 text-gray-500">Tidak ada permohonan virtual meeting ditemukan</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
    <div class="px-5 py-4 border-t border-slate-100 flex justify-end">
      {{ $vm->appends(request()->query())->links() }}
    </div>
  </div>
  ...
  <script>
    var vmData = @json($vm);
    var updateRouteBase = "{{ route('update.vm.admin', ':id') }}";
    var uploadBaseUrl = "{{ url('/uploads/') }}/";

    function openModalPelaksanaan(id) {
      var item = vmData.find(x => x.id == id);
      if(!item) return;
      document.getElementById('modal-title').innerText = 'Detail Pelaksanaan - ' + item.nomor_tiket;
      ...
      document.getElementById('modal-content').innerHTML = `
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div><p class="text-xs text-gray-500 mb-1">Topik Meeting</p><p class="font-medium text-gray-800">${item.topik_meeting}</p></div>
            <div><p class="text-xs text-gray-500 mb-1">Waktu Pelaksanaan</p><p class="font-medium text-gray-800">${item.waktu_pelaksanaan}</p></div>
            <div><p class="text-xs text-gray-500 mb-1">Jumlah Partisipan</p><p class="font-medium text-gray-800">${item.jumlah_partisipan}</p></div>
            <div><p class="text-xs text-gray-500 mb-1">Durasi</p><p class="font-medium text-gray-800">${item.durasi_meeting}</p></div>
            <div class="col-span-2"><p class="text-xs text-gray-500 mb-1">Lokasi</p><p class="font-medium text-gray-800">${item.lokasi_meeting}</p></div>
        </div>
    ...
    function closeModal() {
        ...
        setTimeout(() => {
            ...
            document.getElementById('modal-content').innerHTML = ''; // clear iframe memory
        }, 200);
    }
