<x-layouts.modern>
  <div class="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h2 class="text-xl font-heading font-bold text-slate-800">Infrastruktur Baru</h2>
      <p class="text-sm text-slate-500">Daftar permohonan infrastruktur jaringan baru.</p>
    </div>
    <div class="flex items-center gap-2 text-sm text-slate-500">
      <span>Home</span> <i data-lucide="chevron-right" class="w-3 h-3"></i> <span class="font-semibold text-slate-700">Infrastruktur Baru</span>
    </div>
  </div>
  ...
  <div id="table-container" class="bg-white rounded-box shadow-soft border border-slate-200 overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
      <h3 class="font-heading font-bold text-sm text-slate-800">Data Infrastruktur Baru</h3>
    </div>
      ...
      <table class="w-full text-left border-collapse data-table">
        <thead>
          <tr>
            ...
            <th class="px-5 py-3 border-b border-slate-100 bg-slate-50/50 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Jenis Koneksi</th>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-xs text-gray-500">{{ $item->jenis_koneksi }}</td>
          ...
          @empty
          <tr><td colspan="7" class="text-center py-4 text-gray-500">Tidak ada permohonan Infrastruktur Baru ditemukan</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
    <div class="px-5 py-4 border-t border-slate-100 flex justify-end">
      {{ $infrastruktur->appends(request()->query())->links() }}
    </div>
  </div>
  ...
  <script>
    var dbData = @json($infrastruktur);
    var updateRouteBase = "{{ route('update.infrastruktur.admin', ':id') }}";
